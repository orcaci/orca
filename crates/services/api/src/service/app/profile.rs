...
use tracing::info;
use uuid::Uuid;
use entity::test::profile::data::{Column as DataColumn, Entity as DataEntity, Model as DataModel};
...
impl ProfileService {
    ...
    pub async fn list_profile(&self) -> InternalResult<Vec<Model>> {
        ...
        let profiles = Entity::find().filter(_filter).order_by_asc(Column::Name).all(self.trx()).await?;
        Ok(profiles)
    }
    ...
    pub async fn update_profile(&self, profile_id: Uuid, mut profile: Model) -> InternalResult<Model> {
        let _profile = Entity::find_by_id(profile_id).one(self.trx()).await?;
        if _profile.is_none() {
            return Err(OrcaRepoError::ModelNotFound("Profile".to_string(), profile_id.to_string()))?;
        }
    ...
    pub async fn delete_profile(&self, profile_id: Uuid) -> InternalResult<()> {
        ...
        info!("Deleting Application Profile Data of ProfileID- {:?}", profile_id);
        let result = DataEntity::delete_many()
            .filter(DataColumn::ProfileId.eq(profile_id))
            .exec(self.trx()).await?;
        info!("Deleted Application Profile Data of ProfileID- {:?}, count - {:?}", profile_id, result.rows_affected);
        Ok(())
    }
    ...
    pub async fn delete_profile_data(&self, _data_id: Uuid) -> InternalResult<()> {
        ...
        if result.rows_affected == 0 {
            return Err(OrcaRepoError::ModelNotFound("Profile Item Data".to_string(), _data_id.to_string()))?;
        }
