...
use sea_orm::{ActiveModelTrait, ColumnTrait, ConnectionTrait, DatabaseBackend, DatabaseTransaction, EntityTrait, IntoActiveModel, JsonValue, ModelTrait, NotSet, QueryFilter, QueryOrder, QueryResult, Statement, TryIntoModel};
use sea_orm::ActiveValue::Set;
use sea_orm_migration::SchemaManager;
use sea_query::{Alias, Asterisk, ColumnDef, Condition, Expr, Order, PostgresQueryBuilder, Query, Table};
...
impl DatatableService {
    ...
    pub async fn list_datatables(&self) -> InternalResult<Vec<Model>> {
        let tables = Entity::find().filter(Column::AppId.eq(self.1))
    ...
    pub async fn get_datatable(&self, id: i32) -> InternalResult<Model> {
        ...
        if tables.is_none() {
            return Err(OrcaRepoError::ModelNotFound("Datatable".to_string(), id.to_string()))?;
        }
        Ok(tables.unwrap())
    }
    ...
    pub async fn create_datatable(&self, mut table: Model) -> InternalResult<Model> {
        ...
        let exec_res= self.trx().execute(Statement::from_string(DatabaseBackend::Postgres, stm)).await?;
        info!("Created the Table - {:?}", exec_res);
        Ok(result)
    }

    /// create_new_field - this will create new Field in Data Application in Orca
    pub async fn create_new_field(&self, mut table_id: i32, mut field: FieldModel) -> InternalResult<FieldModel> {
        ...
        let res = manager.alter_table(
        Table::alter().table(Alias::new(table.table_name))
    ...
    pub async fn drop_field(&self, table_id: i32, field_id: String) -> InternalResult<()> {
        ...
        let manager = SchemaManager::new(self.trx());
        manager.alter_table(
            Table::alter().table(FieldEntity)
                .drop_column(Alias::new(field_id.clone())).to_owned()
        ).await?;
        let result = FieldEntity::delete_many().filter(_filter).exec(self.trx()).await?;
    ...
    pub async fn batch_update_data(&self, table_id: i32,
                                   mut table_datas: Vec<TableDataRequest>) -> InternalResult<()> {
        let table = self.get_datatable(table_id).await?;
        ...
        for data in table_datas {
            ...
            self.trx().execute(Statement::from_string(DatabaseBackend::Postgres,
                                                      query)).await?;
        }
        Ok(())
    }
    ...
    pub async fn update_data(&self, table_id: i32, value: Value) -> InternalResult<()> {
        ...
        let query = Query::insert()
            .into_table(Alias::new(table.table_name.clone())).or_default_values()
            .to_owned().to_string(PostgresQueryBuilder);
        self.trx().execute(Statement::from_string(DatabaseBackend::Postgres,
                                                      query)).await?;
        Ok(())
    ...
    pub async fn delete_table(&self, table_id: i32) -> InternalResult<()> {
        ...
        if table.is_none() {
            return Err(OrcaRepoError::ModelNotFound("Table".to_string(), table_id.to_string()))?;
        }
        let table = table.unwrap();
        let manager = SchemaManager::new(self.trx());
        let res = manager.drop_table(
        Table::drop().table(Alias::new(table.table_name.clone()))
            .to_owned()
        ).await?;
