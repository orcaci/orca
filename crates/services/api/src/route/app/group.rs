...
pub(crate) fn group_route() -> Router {
    Router::new()
        ...
        .route("/:group_id",put(update_action_group).delete(delete_action_group))
}

/// get_groups - list all the Action Group in Specific Application in the Orca Application
async fn get_groups(Extension(session): Extension<OrcaSession>,
...
async fn create_group(Extension(session): Extension<OrcaSession>, Path(app_id): Path<Uuid>,
                       Json(body): Json<Model>) -> InternalResult<impl IntoResponse> {
    let result = GroupService::new(session).create_group(app_id, body).await?;
    Ok((StatusCode::CREATED, Json(result)))
}

/// update_action_group - this will create new Application in Orca
async fn update_action_group(Extension(session): Extension<OrcaSession>,
                             Path((_app_id, group_id)): Path<(Uuid, Uuid)>,
                             Json(body): Json<Model>) -> InternalResult<impl IntoResponse> {
    let result = GroupService::new(session).update_action_group(group_id, body).await?;
    Ok(Json(result))
}

/// delete_action_group - this will delete Action Group in Application Application in Orca
async fn delete_action_group(Extension(session): Extension<OrcaSession>,
