...
async fn get_action(Extension(session): Extension<OrcaSession>,
                    Path((_app_id, group_id)): Path<(Uuid, Uuid)>) -> InternalResult<impl IntoResponse> {
...
async fn create_action(Extension(session): Extension<OrcaSession>, Path((_app_id, group_id)): Path<(Uuid, Uuid)>,
                       Json(body): Json<Model>) -> InternalResult<impl IntoResponse> {
    let result = ActionService::new(session).create_action(group_id, body).await?;
...
async fn update_action(Extension(session): Extension<OrcaSession>,
                       Path((_app_id, _group_id, action_id)): Path<(Uuid, Uuid, Uuid)>,
                       Json(body): Json<Model>) -> InternalResult<impl IntoResponse> {
    let result = ActionService::new(session).update_action(action_id, body).await?;
    Ok(Json(result))
}

/// delete_action - this will delete Action for Action Group in Application in Orca
async fn delete_action(Extension(session): Extension<OrcaSession>,
                       Path((_app_id, _group_id, action_id)): Path<(Uuid, Uuid, Uuid)>) -> InternalResult<impl IntoResponse> {
