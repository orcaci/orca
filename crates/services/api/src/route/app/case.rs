...
async fn list_cases(Extension(session): Extension<OrcaSession>,
...
async fn create_case(Extension(session): Extension<OrcaSession>, Path(app_id): Path<Uuid>,
...
async fn get_case_info(Extension(session): Extension<OrcaSession>,
                        Path((app_id, case_id)): Path<(Uuid, Uuid)>) -> InternalResult<impl IntoResponse> {
    let result = CaseService::new(session, app_id).get_case_info(case_id).await?;
    Ok(Json(result))
}


/// dry_run - Dry run the Test case
async fn dry_run(Extension(session): Extension<OrcaSession>,
                        Path((app_id, case_id)): Path<(Uuid, Uuid)>) -> InternalResult<impl IntoResponse> {
...
async fn update_block(Extension(session): Extension<OrcaSession>,
                           Path((app_id, case_id)): Path<(Uuid, Uuid)>,
                           Json(body): Json<Vec<BlockModel>>) -> InternalResult<impl IntoResponse> {
    let result = CaseService::new(session, app_id).batch_update_case_block(case_id, body).await?;
    Ok(Json(result))
}


/// insert_block - This will Append New Block to the code for spe
async fn insert_block(Extension(session): Extension<OrcaSession>,
                           Path((app_id, case_id)): Path<(Uuid, Uuid)>,
                           Json(body): Json<BlockModel>) -> InternalResult<impl IntoResponse> {
    let result = CaseService::new(session, app_id).push_block(case_id, body, None, None).await?;
    Ok(Json(result))
}
