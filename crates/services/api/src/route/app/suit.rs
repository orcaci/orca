...
async fn list_suites(Extension(session): Extension<OrcaSession>,
...
async fn create_suite(Extension(session): Extension<OrcaSession>, Path(app_id): Path<Uuid>,
...
/// get_suits_info - Get Suite Info and the batch information with the list of block
async fn get_suite_info(Extension(session): Extension<OrcaSession>,
                        Path((app_id, suite_id)): Path<(Uuid, Uuid)>) -> InternalResult<impl IntoResponse> {
    let result = SuitService::new(session, app_id).get_suite_info(suite_id).await?;
    Ok(Json(result))
}
...
async fn insert_block(Extension(session): Extension<OrcaSession>,
    ...
    let result = SuitService::new(session, app_id).push_block(suite_id, body, None).await?;
    Ok(Json(result))
}
...
async fn update_block(Extension(session): Extension<OrcaSession>,
    ...
    let result = SuitService::new(session, app_id).batch_update_suite_block(suite_id, body).await?;
    Ok(Json(result))
}
