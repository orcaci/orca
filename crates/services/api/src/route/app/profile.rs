...
async fn create_profile(
    ...
    let result = ProfileService::new(session, app_id)
        .create_profile(body)
        .await?;
...
async fn delete_profile(
    ...
    let _result = ProfileService::new(session, app_id)
...
async fn delete_profile_data(
    Extension(session): Extension<OrcaSession>,
    Path((app_id, _profile_id, data_id)): Path<(Uuid, Uuid, Uuid)>,
) -> InternalResult<impl IntoResponse> {
    let _result = ProfileService::new(session, app_id)
        .delete_profile_data(data_id)
        .await?;
    Ok(Json(json!({"status": "success"})))
}
